  ...
  <div class="app-container">
    ...
    <el-dialog title="用户登录日志" :visible.sync="dialogTableVisible">
      <el-table :data="loginRecordList" border stripe>
        ...
        <el-table-column prop="createTime" label="登陆时间"></el-table-column>
      </el-table>
    </el-dialog>
  </div>
</template>
<script>
import api from '@/api/core/user-info.js'
export default {
  data() {
    return {
      list: null,
  ...
  methods: {
    async fetchData() {
      ...
      this.list = res.pageModel.records
    ...
    async lock(id, status) {
      const response = await api.lock(id, status)
    ...
    async showLoginRecord(id) {
      ...
      const response = await api.getUserLoginRecordTop50(id)
